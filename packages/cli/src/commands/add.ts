...
export const addCommand = defineCommand({
  ...
  async run({ args }) {
    ...
    if (args.dryRun) {
      ...
      console.log(`    ${colors.dim("1.")} login       ${colors.dim("(prompt for PAT or run OAuth)")}`);
      console.log(
      ...
      console.log(`    ${colors.dim("3.")} materialize ${colors.dim("fetch credentials for the resource")}`);
      console.log(`    ${colors.dim("4.")} persist     ${colors.dim("write secrets to Phantom + MCP + .stack.toml")}`);
      console.log();
