/**
 * Mixpanel product analytics health-check probe.
 *
 * Calls GET /api/app/me to validate the API secret and surface account info.
 * Mixpanel uses HTTP Basic auth with the API secret as username and empty password.
 *
 * Reference: https://developer.mixpanel.com/reference/overview
 * Dashboard:  https://mixpanel.com
 *
 * Credentials required: MIXPANEL_API_SECRET
 */
...
const probe: Probe = {
  provider: PROVIDER,
  label: "Mixpanel API health-check",

  async run(ctx: ProbeContext): Promise<ProbeResult> {
    const apiSecret = await tryRevealSecret("MIXPANEL_API_SECRET");
    if (!apiSecret) {
      return {
        provider: PROVIDER,
        ...
        latencyMs: 0,
        status: "skipped",
        detail: "MIXPANEL_API_SECRET not in vault — probe skipped",
      };
    }

    // Mixpanel uses HTTP Basic auth with API secret as username, empty password.
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: `Mixpanel /api/app/me returned HTTP ${res.status} — check MIXPANEL_API_SECRET`,
        };
      }
      ...
      if (res.status === 429) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: "Mixpanel API rate-limited (HTTP 429)",
        };
      }
      ...
      if (!res.ok) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: `Mixpanel /api/app/me returned HTTP ${res.status}`,
        };
      }
      ...
      return {
        ...
        status: "error",
        detail: `Mixpanel probe failed: ${(err as Error).message}`,
      };
    }
  },
};
