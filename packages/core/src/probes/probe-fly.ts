/**
 * Fly.io VMs-at-the-edge health-check probe.
 *
 * Calls GET /v1/apps to validate the API token and surface rate-limit headers.
 *
...
const probe: Probe = {
  provider: PROVIDER,
  label: "Fly.io API token health-check",

  async run(ctx: ProbeContext): Promise<ProbeResult> {
    const token = await tryRevealSecret("FLY_API_TOKEN");
    if (!token) {
      return {
        provider: PROVIDER,
        ...
        latencyMs: 0,
        status: "skipped",
        detail: "FLY_API_TOKEN not in vault — probe skipped",
      };
    }

    const start = Date.now();
    try {
      const res = await fetch("https://api.fly.io/v1/apps?page=1&per_page=1", {
        headers: {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: `Fly.io /v1/apps returned HTTP ${res.status} — check FLY_API_TOKEN`,
        };
      }
      ...
      if (res.status === 429) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: "Fly.io API rate-limited (HTTP 429)",
        };
      }
      ...
      if (!res.ok) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: `Fly.io /v1/apps returned HTTP ${res.status}`,
        };
      }
      ...
      return {
        ...
        status: "error",
        detail: `Fly.io probe failed: ${(err as Error).message}`,
      };
    }
  },
};
