/**
 * Postmark transactional email health-check probe.
 *
 * Calls GET /servers to validate the account token and surface rate-limit headers.
 *
 * Reference: https://postmarkapp.com/developer/api/overview
 * Dashboard:  https://account.postmarkapp.com
 *
 * Credentials required: POSTMARK_ACCOUNT_TOKEN
 */
...
const probe: Probe = {
  provider: PROVIDER,
  label: "Postmark account token health-check",

  async run(ctx: ProbeContext): Promise<ProbeResult> {
    const token = await tryRevealSecret("POSTMARK_ACCOUNT_TOKEN");
    if (!token) {
      return {
        provider: PROVIDER,
        ...
        latencyMs: 0,
        status: "skipped",
        detail: "POSTMARK_ACCOUNT_TOKEN not in vault — probe skipped",
      };
    }

    const start = Date.now();
    try {
      const res = await fetch("https://api.postmarkapp.com/servers?count=1&offset=0", {
        headers: {
      ...
      if (res.status === 401 || res.status === 403) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: `Postmark /servers returned HTTP ${res.status} — check POSTMARK_ACCOUNT_TOKEN`,
        };
      }
      ...
      if (res.status === 429) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: "Postmark API rate-limited (HTTP 429)",
        };
      }
      ...
      if (!res.ok) {
        return {
          ...
          latencyMs,
          status: "error",
          detail: `Postmark /servers returned HTTP ${res.status}`,
        };
      }

      const body = (await res.json()) as { TotalCount?: number; Servers?: unknown[] };
      const serverCount = body.TotalCount ?? body.Servers?.length ?? 0;

      // Postmark rate-limit headers.
      ...
      const result: ProbeResult = {
        ...
        latencyMs,
        status,
        detail: `Postmark API reachable (${serverCount} server(s))`,
      };
      ...
      return {
        ...
        status: "error",
        detail: `Postmark probe failed: ${(err as Error).message}`,
      };
    }
  },
};
