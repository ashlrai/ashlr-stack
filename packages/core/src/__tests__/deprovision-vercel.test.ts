...
describe("vercel deprovision", () => {
  let h: Harness;
  let realFetch: typeof fetch;

  const ctx: ProviderContext = { cwd: process.cwd(), interactive: false, log: () => {} };
  const auth = { token: "vercel_valid_token", identity: { uid: "u1" } };
  ...
  test("success — DELETE returns 200, resolves without error", async () => {
    globalThis.fetch = (async (url: string | URL | Request, init?: RequestInit) => {
      ...
      if (u.includes("/v9/projects/prj_abc") && method === "DELETE") {
        return new Response("", { status: 200 });
      }
      throw new Error(`unexpected fetch ${method} ${u}`);
    }) as typeof fetch;

    await expect(vercel.deprovision!(ctx, auth, "prj_abc")).resolves.toBeUndefined();
  });
  ...
  test("non-ok response logs warn but does not throw", async () => {
    ...
    await expect(vercel.deprovision!(warnCtx, auth, "prj_abc")).resolves.toBeUndefined();
    expect(logs.some((l) => l.includes("prj_abc"))).toBe(true);
  });
