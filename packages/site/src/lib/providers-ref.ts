/**
 * Provider reference data — one entry per curated provider.
 *
 * Mirrored from `packages/core/src/providers/*.ts`. When a provider's metadata
 * changes there (authKind, secret slots, MCP wiring), update the matching
 * entry here so the docs stay accurate.
 *
 * The separate simple-icons-backed catalogue at `./providers.ts` is what the
 * landing page renders; this file is the source of truth for /docs/providers.
 */

export type AuthKind = "oauth_pkce" | "oauth_device" | "pat" | "api_key";
